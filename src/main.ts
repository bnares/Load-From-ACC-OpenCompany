...
import { RvtUI } from "./bim-components/RVTLoader/src/user-interface";
import { RVTLoader } from "./bim-components/RVTLoader";
...
const revitLoader = components.get(RVTLoader);
const authCode = revitLoader.getQuerryParameters("code"); //at first loading this will return null but when we get through autentication processwe will have in aour querry params param with name code

if(authCode){
  await revitLoader.getToken(authCode);
}

revitLoader.onProcessingFinished.add((model)=>{
  world.scene.three.add(model);
})
